...
class TruffleCompiler(ctx: MontunoContext) : Compiler(ctx) {
    ...
    override fun buildClosure(t: Term, env: VEnv): Closure {
        ...
        val body = compile(bodyTerm, Lvl(env.it.size))
        return TruffleClosure(ctx, env.it, heads, heads.size, body)
    }
...
//        val (heads, _) = stripHeads(t)
//        return TruffleClosure(ctx, emptyArray(), heads, heads.size, callTarget)
//    }
    ...
    private fun compileTerm(t: Term, depth: Lvl, fd: FrameDescriptor): Code = when (t) {
        is TPi -> {
            ...
            CClosure(t.name, t.icit, compileTerm(t.body, depth, fd), true, ctx, heads, compile(body, depth + 1), null)
        }
        is TLam -> {
            val (heads, body) = stripHeads(t.body)
            CClosure(t.name, t.icit, compileTerm(t.body, depth, fd), false, ctx, heads, compile(body, depth + 1), null)
        }
