...
inline class VSpine(val it: Array<Pair<Icit, Val>>) // lazy ref to Glued
inline class VEnv(val it: Array<Val?>)  // LAZY
...
data class GluedVal(val v: Val, val g: Glued)
...
fun VEnv.def(x: Val) = VEnv(it.plus(x))
...
fun gvLocal(ix: Ix) = GluedVal(vLocal(ix), gLocal(ix))
