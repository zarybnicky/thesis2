...
* Language Design $\equiv$ Designing a DTLC
** Dependent types
...
Π(n:ℕ). Vec(ℝ, n) ~ dependent function type
Π(n:ℕ). ℝ ~ regular function type
...
* Language Implementation $\equiv$ Implementing a DTLC: Montuno
...
* Evaluation methodology $\equiv$ Designing DTLC benchmarks
...
* Introducing gradual just-in-time compilation \equiv Adding Truffle to Montuno: $Montuno_{\mathbb{T}}$
...
* Optimizations $\equiv$ Making $Montuno_{\mathbb{T}}$ fast

* Results $\equiv$ Is $Montuno_{\mathbb{T}}$ fast enough?
