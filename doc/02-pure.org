...
Scala or Kotlin, as the code is often cleaner and more concise[fn:3], so in the
...
#+LABEL:program-flow
#+CAPTION: Interpreter component overview
...
#+LABEL:data-flow
#+CAPTION: Data flow overview. Cyan is elaboration, red is normalization-by-evaluation
#+ATTR_LATEX: :options [!htb]
#+begin_figure latex
\centering
\begin{tikzpicture}[
...
compiler GHC, it produces code an order of magnitude faster than using other
...
though, we will only describe the relevant parts of the specification in words, and
...
pair of terms, which is given by the outermost constructor, the /head/.
...
has as its input \texttt{PreTerm}s, and produces \texttt{Term}s in the case of
...
without a placeholder, so we are not able to infer the type of the body of the
...
The grammar from the previous chapter is translated into ANTLR grammar almost
